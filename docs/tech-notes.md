...
- **Shell commands.** Families look past wrappers and their options (`sudo -u`, `nice -n`, `timeout -s KILL 60`, `xargs -n`, `env -u`), subshells and `{ }` groups. Saver command routes look past only what rtk's hook does (`rtk rewrite`, rtk 0.50.0): env assignments, `exec`/`command`/`builtin`/`noglob`/`nocorrect`, and `timeout`, `time`, `nice` and `nohup` with the options rtk knows. `sudo`, `xargs`, `env -u`, `command -v`, `(cmd)` and `{ cmd; }` are left alone, as rtk leaves them, so rtk is not credited for them. In a pipeline, routes see the stage rtk rewrites: a final `grep` or `rg`, or else the first program when the rest is only `head`, `cat` or a `tail` that does not follow, and not then for `tsc`, `vitest`, `ctest` or `sqlfluff`, which rtk leaves alone when piped. Other pipelines (`| grep … | head`, `| wc -l`, `| sort`, `| tee`), and pipes in a command with a subshell or `{ }` group, are not credited; nor is a command with `$( )`, backticks or a heredoc, which rtk leaves alone too. `||` ends a segment like `&&`. rtk's routes match only the forms its hook rewrites to that filter: not `cargo nextest`, `yarn tsc`/`vitest`, `egrep`, `fd`, bare `ruff`, bare `phpstan` or `pytest-watch`, nor `bunx vitest` (rtk runs bunx's generic filter there); `./vendor/bin/…` and `pnpm prettier` do match. Both join continued lines. Package runners (`npx X`, `pnpm exec X`, `uv run X`) are classified by `X`.
...
- **Quick mode.** Extrapolation ratios come only from the hash-order sample of smaller outputs (or from every measured output when all are among the largest). Fewer than 20 usable outputs gives no number. So do measured outputs that are exactly the larger ones of those a ratio comes from, with three or more left unmeasured (a cache from an older release or a longer period, stopped part-way); one or two unmeasured outputs, such as one new since an exact run or one whose replay failed, are extrapolated even when they happen to be the smallest. Persisted-output previews are always measured first but never set a ratio.
...
- Each run stores its draw per saver next to the replay cache (`quick-draws-v1.json`, hashes and numbers only): the salt, the cache fingerprint once every saver in the run has written, each output's size, and the outputs drawn under that salt. Outputs are keyed by the first 8 characters of their content hash; sizes are kept on a log scale to 0.3% in 3 characters. That is 11 characters an output: 427 KB for token-saver and lean-ctx on the author's 30 days (about 20k outputs each). The store is written only for savers with estimated outputs.
- **Reuse rule.** The next run reuses the salt when nothing has written the cache since the stored run ended and the populations overlap by at least 90% of their size mass both ways: at least 90% of the new population's Σx is on outputs the old one had, and at least 90% of the old one's Σx is on outputs still present. Outputs drawn under the salt are not treated as cached when planning, so they stay in the population. Those the new plan samples are its sample again, with no replay. Those it does not sample are exact but set no ratio. New outputs join the draw, so a few may be replayed. Anything else draws afresh: a larger change in the logs or the window, an exact run, or another run that wrote the cache.
- Because the plan still draws over the population minus the outputs cached when the salt was made, the sample stays a probability sample of the same kind. The only cost is that drawn outputs falling out of the sample count as exact without setting the ratio.
- `SAVER_AUDIT_STRICT_SAMPLE=1` never uses the store. A corrupt store is ignored, one that cannot be written gives one warning, and it is not written when the cache was not saved.
- **Simulator** (the judge's warm-cache scenarios, run through the product's `planQuick`/`fitQuick`/`populationOverlap`, 100 seeds, token Σd error). Added scenarios:
  - the month run three times;
  - rolling 30-day windows (dumped for 30 days ending 09-22, 09-23, 09-24 and 09-25 at 13:00) moved by 1 day, by 2 days, and by 1 day with each window run twice.

  | Scenario | Rule | token-saver bias | lean-ctx bias | caveman bias | headroom bias | Runs that reused |
  |---|---|---|---|---|---|---|
  | all 11 judge scenarios | fresh draw whenever the cache changed | −2.0 to +2.2% | −1.2 to +0.9% | −1.4 to +1.1% | −1.0 to +1.0% | 0% |
  | all 11 judge scenarios | 90% reuse | −2.6 to +2.2% | −1.2 to +0.2% | −1.2 to +1.1% | −0.4 to +1.5% | 0–67% |
  | month ×3 | 90% reuse | −2.0% | +0.2% | +1.1% | −0.4% | 67% (runs 2 and 3) |
  | rolling 30d, 1-day moves | 90% reuse | +0.3% | −0.5% | +0.1% | −0.5% | 25% |
  | rolling 30d, 1-day moves, twice a day | 90% reuse | +0.3% | −0.5% | +0.1% | −0.5% | 63% |
  | rolling 30d, 2-day moves | 90% reuse | +0.6% | −0.8% | −1.4% | +0.1% | 0% |

  Every scenario stays within ±3% for every saver. On these logs a 1-day move of the 30-day window often shifts more than 10% of the size mass (the last days are the heaviest), so it draws afresh; a 2-day move always does. Median errors are unchanged, because a reused draw is as good as a fresh one. What reuse gives up is the drift toward exact that repeated fresh draws have as they cache more outputs (caveman, twice a day: RMSE 2.6% fresh vs 9.7% reused).
- **Real check, fixed window** (empty cache, `--since 2026-08-26 --until 2026-09-25T13:00 --json`, default savers, four runs in a row): token-saver $70.41 and lean-ctx $156.16 every time (±21% and ±16%). Run 1 replayed 270 outputs per saver in 23.8 s; runs 2–4 replayed none, in 12.0, 11.7 and 10.6 s (load average about 19).
- **Real check, default period** (no `--since`/`--until`, empty cache, three runs in a row): token-saver $95.06 and lean-ctx $192.20 each time (±25%, ±18%). Run 1 took 25.3 s and replayed 270 per saver; runs 2 and 3 took 10.4 and 9.9 s and replayed none. Before this rule, three such runs gave $95.07 → $96.86 → $101.68 and $191.52 → $172.75 → $184.98, with 270 replays each. Moving a 30-day window's end by two hours (`--last 30d --until 2026-09-25T11:00`, then `T13:00`) reused the draw: 1 replay, token-saver $69.60 → $69.56, lean-ctx $152.84 → $153.97. Moving it by a day (to `09-26T13:00`, with a heavy day added) drew afresh.

**Review fixes** (an independent review of the quick-mode changes, each with a regression test):
- **Certainties crowding the ratio sample.** Just above the budget (280 outputs at 270), most outputs are certainties and fewer than 20 were left to sample, so there was no number even though almost everything had been replayed. The plan now grows the budget a little: with fewer than 20 outputs left unsampled it replays them all, and it adds 20 at a time until at least 20 non-certain outputs are sampled. When the clock stops among the certainties anyway, what is left gets the measured outputs' plain ratio per size band if it is at most a quarter of the saver's size (as for cache-only savers: indicative, share stated, no ±); otherwise no number.
- **Calls on unpriced models.** An output read only by calls on unpriced models had size $0, so it was never replayed or estimated, and the saver still read "exact" while its token column came out low (a synthetic fixture: rtk 460.7 tokens quick, 920.7 exact). `saverBlockWeights` now also gives each block its context-token weight and the unpriced part of it. An output has size 0 only when no call in the period reads it; such an output counts in neither column. A read at $0 is taken at the saver's mean dollars per priced token. If more than 5% of a saver's token weight is unpriced, quick mode works in context tokens instead, and the label says so.
- **The short view's ±** is now computed for the number on the row: for a saver whose Codex part is hypothetical, over the part outside Codex files (the same π and ratios, that part's savings and sizes).
- **A sample with no spread** (se 0, usually no sampled output changed) no longer prints "±1%": the report says no range can be given, and why.
- **The note's counts** now give the outputs actually measured (cached or replayed, not failed) and how many were estimated from them.
- **The drawn set** kept under a reused salt is limited to outputs still in the population.
- **Checks after the fixes:** the month and the four weeks, quick vs exact in dollars, are unchanged (token-saver +7.0%, +4.9%, exact, exact, −24.1%; lean-ctx +24.4%, +3.9%, exact, +4.4%, +6.3%). Three default runs from an empty cache: token-saver $94.99 (±25%) and lean-ctx $198.75 (±18%) each time; 30.4 s for the first run, then 9.9 and 9.4 s with no replays. The totals moved from earlier runs because the default period ends now.
