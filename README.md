...
- **replayed.** saver-audit sends the recorded tool output through *your installed copy* of the saver and counts what is left, measured against what the model actually saw.
  ...
  - **Quick by default:**
    ...
    - A quick run keeps its sample: running again over the same period with nothing new gives the same numbers and replays nothing.
  ...
  - **Exact on request:** press `e` or pass `--exact`.
    ...
    - Results are cached and count as exact in later quick runs. Outputs new since then are sampled again, or, for the caveman engine and headroom, handled as above.
