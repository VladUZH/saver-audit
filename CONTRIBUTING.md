...
| `lowerBound` | `true` if your saver also cuts things offline replay cannot see (e.g. commands it re-runs itself). The number is then shown as a floor: "lower bound" in the report and on the card, "at least" in the post. Say what is missed in `assumption`. |
