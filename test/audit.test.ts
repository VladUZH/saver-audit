...
//   fork (after marker) terra: 300×2 + 30×12                       = 960
const EXPECTED = (9012 + 3308 + 470 + 305 + 3200 + 3800 + 32 + 960) / 1e6 + 0.02;
...
test("fixture totals match hand-computed, deduplicated usage", async () => {
  ...
  assert.equal(r.skipped.notBillable, 1, "forked thread's replayed history");
  assert.equal(r.skipped.lines, 2);
  assert.deepEqual(r.sessions, { main: 3, subagent: 1, bySource: { "claude-code": 1, codex: 2 } });
...
test("period filter drops calls outside the window", async () => {
  ...
  assert.equal(r.calls, 1, "only the forked thread's own call on 09-21");
});
